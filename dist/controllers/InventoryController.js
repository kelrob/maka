...
class InventoryController {
    store(req, res) {
        try {
            ...
            if (!Array.isArray(inventory))
                return utils.formatResponse(res, 400, false, "Invalid Data Submitted. Cannot Process request.");
            ...
            inventory.forEach((item) => {
                if (Number.isInteger(item.quantity) && Number.isInteger(item.itemID)) {
                    if (utils.itemIDExists(item.itemID, inventories)) {
                        let itemIndex = inventories.findIndex((key) => key.itemID === item.itemID);
                    ...
                    utils.formatResponse(res, 400, false, `Invalid data type pased for "quantity" or "itemID"`);
                }
            });
            // Show latest Inventory
            utils.formatResponse(res, 201, true, "Item Added Successfully", inventories);
        }
        catch (error) {
            ...
            if (utils.itemIDExists(itemID, inventories)) {
                ...
                if (inventories[itemIndex].quantity > 0) {
                    ...
                    if (utils.itemIDExists(itemID, soldItems)) {
                        const itemIndex = soldItems.findIndex((key) => key.itemID === itemID);
                    ...
                    utils.formatResponse(res, 201, true, "Item bought successfully!", soldItems);
                }
                else {
    ...
    soldItems(req, res) {
        try {
            ...
            if (utils.itemIDExists(itemID, soldItems) == false)
                return utils.formatResponse(res, 400, false, "ItemID not found in sold Items");
            ...
            if (utils.showIDExists(showID, soldItems)) {
                soldItems.forEach((item, index) => {
                    if (item.itemID === itemID && item.showID === showID) {
                        utils.formatResponse(res, 200, true, "Data Retrieved successfully", {
    ...
    soldItemsByShow(req, res) {
        try {
            let result = [];
            const showID = parseInt(req.params.show_id);
            if (utils.showIDExists(showID, soldItems) == false)
                return utils.formatResponse(res, 400, false, "showId not found in sold Items");
            soldItems.forEach((item) => {
                if (item.showID === showID) {
                    result.push(item);
                }
            });
            utils.formatResponse(res, 200, true, "Data retrieved successfully", result);
        }
        catch (error) {
            utils.formatResponse(res, 500, false, error.toString());
        }
